...
public class StringUtil {
    ...
    static public String stringValue(String v, String def) {
        if (v == null || v.trim().length() == 0 || "null".equals(v)) {
            return def;
        }
        return v.trim();
    }

    static public String[] stringArrayValue(String[] v, String[] def) {
        if (v == null || v.length == 0) {
            return def;
        }
        return v;
    ...
    public static String getUTF8StringFromIOS(String v , String def) throws UnsupportedEncodingException{
    	if (v == null || v.trim().length() == 0) {
            return def;
        }
    	
    	byte bb[];
        bb = v.getBytes("ISO-8859-1"); //以"ISO-8859-1"方式解析name字符串
        v= new String(bb, "UTF-8"); //再用"utf-8"格式表示name
        return v;
    }
}